...
function keyPressed() {
  if (document.getElementById("res-ei") != null) {
    const { display } = window.getComputedStyle(
      document.getElementById("res-ei")
    );
    mobile = display == "none";
  }
...
async function getData() {
  ...
  try {
    const response = await fetch(
      `https://api.muchas-glacias.com/results-logs/${id}`,
      {
        method: "GET",
      }
    );
...
const populate = (res) => {
  ...
  let score = [
    ["ei", res["eco-index"]],
    ["perf", res["lighthouse-perf"]],
    ["acc", res["lighthouse-accessibility"]],
    ["bp", res["lighthouse-best-practices"]],
    ["seo", res["lighthouse-seo"]],
  ];

  score.forEach((item) => {
    if (mobile) gauge(item[0], item[1]);
    else pills(item[0], item[1]);
  });
};
...
const launch = async (url) => {
  ...
  if (!checkURL(url)) alert("Veuillez entrer une URL valide");
  else {
...
const rebootData = () => {
  ...
  document
    ...
    .getElementsByClassName("txt")[0].innerHTML = "Veuillez patienter...";
  document
    .getElementById("waiting")
    .getElementsByClassName("material-icons-outlined")[0].innerHTML = "refresh";
  ...
  elts.forEach((elt) => {
    ...
    if (mobile) {
      document
        .getElementById(resItem)
        .getElementsByClassName("slime")[0].style.width = "0";
...
const getScores = async (url) => {
  ...
  try {
    ...
    const responses = await Promise.all([
      ecoindexNcarbon(url),
      ...(await googleLH(url, lhCategoriesAndIds)),
      url,
    ]);
    return responses;
  } catch (err) {
...
const showErr = (e) => {
  ...
  document
    .getElementById("waiting")
    .getElementsByClassName("txt")[0].innerHTML =
    "Une erreur s'est produite, veuillez vérifiez l'url et recommencez";
  document
    .getElementById("waiting")
    .getElementsByClassName("material-icons-outlined")[0].innerHTML = "error";
};
...
const setData = async (data, piScore, piGrade) => {
  try {
    const response = await fetch(
      "https://api.muchas-glacias.com/results-logs",
      {
        method: "POST",
...
const showLink = (id) => {
  document
    .getElementById("link2Share")
    .setAttribute("href", `/score/?id=${id}`);
...
const showCarbon = (carbon) => {
  ...
  select.querySelector("p:last-of-type span").textContent =
    Math.floor(carbon * 100) / 100;
};

// Asking LightHouse
const googleLH = async (url, requestCategories, id) => {
  const req = setUpQuery(url, requestCategories);
  ...
  try {
    ...
    let scores = [];
    for (const [category, id] of Object.entries(requestCategories)) {
      const score = parseInt(lhRes.categories[category].score * 100);
      scores.push(score);
      if (mobile) gauge(id, score);
      else pills(id, score);
    }
...
const setUpQuery = (url, requestCategories) => {
  const api =
...
const pills = (id, score) => {
  if (score === null) {
    console.error("error: score is null");
    return;
  }

  const color = score >= 75 ? "green" : score >= 25 ? "yellow" : "red";

  //Define specific SS rules
  let ss = document.getElementById("animationSS").sheet;
  let extend = 16.5 * (score / 100) - 5.25;
  ss.insertRule(
    `@keyframes level-up-${id.toUpperCase()} {from {height:0rem;} to {height: ${extend}rem}}`,
    ss.cssRules.length
  );
...
const gauge = (id, score) => {
  if (score === null) console.error("error : score is null");
  else {
    color = score >= 75 ? "green" : score >= 25 ? "yellow" : "red";
...
function setCookie() {
  ...
  var cookieLB =
    "labanqui.se_cookie=cookie-consent-labanqui.se;path=/; " +
    expires +
...
window.onload = function () {
  ...
  if (document.cookie.match(cookieName)) {
    ...
    if (styfont == "OpenDys") document.body.classList.add("opendys");
  } else {
  ...
  if (document.cookie.match(cookieName2)) {
    ...
    if (stysize != 100 && stysize >= 90 && stysize <= 200)
      ...
      document.cookie =
        "cookie_labanqui.se_size=100; path=/; " +
        expires +
        "; Secure; sameSite=None";
  }
};
...
function changeFont() {
  if (document.body.classList.contains("opendys")) {
    document.body.classList.remove("opendys");
    document.cookie =
      "cookie_labanqui.se_font=Poppins, Verdana, sans-serif; path=/; " +
      expires +
      "; Secure; sameSite=None";
  } else {
    document.body.classList.add("opendys");
    document.cookie =
      "cookie_labanqui.se_font=OpenDys; path=/; " +
      expires +
      "; Secure; sameSite=None";
  }
}
...
function changeSizeFont(param) {
  ...
  if (size2 != size && size2 <= 200 && size2 >= 90) size = size2;
  else ht.remove("fz" + size);
...
function initialVersion() {
  if (document.body.classList.contains("opendys")) {
    document.body.classList.remove("opendys");
    document.cookie =
      "cookie_labanqui.se_font=Poppins, Verdana, sans-serif; path=/; + expires +; Secure; sameSite=None";
  }
